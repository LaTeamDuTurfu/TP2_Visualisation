...
CURRENT_API_KEY = API_KEY0

def recherche_stock(keyword):
    url = f'https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords={keyword}&apikey={CURRENT_API_KEY}'
...
def get_data_monthly(symbol):
    url = f'https://www.alphavantage.co/query?function=TIME_SERIES_MONTHLY_ADJUSTED&symbol={symbol}&apikey={CURRENT_API_KEY}'
    data = requests.get(url).json()
    return data


def get_data_30_days(symbol):
    url = f'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol={symbol}&outputize=compact&apikey={CURRENT_API_KEY}'
    data = requests.get(url).json()
    return data
